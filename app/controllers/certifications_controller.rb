class CertificationsController < ApplicationController
  ...
  def index
    @certifications = Certification.all.order(:created_at).reverse_order
  end
  ...
  def create
    ...
    respond_to do |format|
      if @certification.save
        format.html { redirect_to '/certificates', notice: 'Correctamente enviado' }
        format.json { render :show, status: :created, location: @certification }
      else
        format.html { redirect_to '/certificates' }
  ...
  private
    ...
    def certification_params
      params.require(:certification).permit(:name, :lastname, :ci, :email, :city, :institution_name, :nit, :phone, :legal_address, :charge, :start_date, :observations, :state)
    end
end
