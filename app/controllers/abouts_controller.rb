class AboutsController < ApplicationController
  ...
  # GET /abouts.json
  def index
    @abouts = About.all.order(:created_at).reverse_order
  end
  ...
  def update
    respond_to do |format|
      if @about.update(about_params)
        format.html { redirect_to abouts_url, notice: 'Nosotros fue actualizado exitosamente' }
  ...
  private
    ...
    def about_params
      params.require(:about).permit(:mission, :vision, :statute_pdf, :document_title, :history, :philosophy)
    end

    def verify_if_user_can_manage?
      redirect_to dashboard_path unless current_user.admin.can_manage_about
    end
